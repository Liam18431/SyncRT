...
namespace SRT
{
	...
	{
		bool result = std::abs(v1 - v2) < eps * std::max(1.0, std::max(std::abs(v1), std::abs(v2)));
	...
	static inline double SampleSpectrum(const std::map<double, double>* const spectrum)
	{
		double result = spectrum->lower_bound(G4UniformRand())->second;
