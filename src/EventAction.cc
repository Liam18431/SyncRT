...
void SRT::EventAction::BeginRun(G4int events_to_process)
{
	...
	SRT::EventAction::total_events_to_process_ = static_cast<G4long>(events_to_process);
	SRT::EventAction::progress_ = 0;
	SRT::EventAction::run_start_tp = static_cast<std::chrono::steady_clock::time_point>(std::chrono::steady_clock::now());
}
...
void SRT::EventAction::BeginOfEventAction(const G4Event*)
{
	G4AutoLock l(&mutex);
	{
		SRT::EventAction::total_events_processed_in_run_ += static_cast<G4int>(1);
	}
}
	...
	if (SRT::EventAction::total_events_to_process_ > 0)
	{
		...
		if (progress % 10 == 0)
		{
			G4AutoLock l(&mutex);
			{
				...
				{
					SRT::EventAction::progress_ = static_cast<G4int>(progress);
